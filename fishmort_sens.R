#this script varies fish mortality inside the loko ia
library(tidyverse)
library(ggpubr)
library(grid)
...
#mortality
g_list<-seq(0, 0.001, by = 0.0001)
g_sequences <- list(0.001, 0.001, g_list)
g_grid <- do.call(expand.grid, g_sequences)
g_list_final <- split(g_grid, 1:nrow(g_grid))
...
for (iter in 1:nrow(parameter_grid)) {
  ...
  for(t in 2:timesteps){
    ...
    fmort[t-1,] = as.numeric(parameter_grid[['g']][[iter]])*fish[t-1,]
...
outcome$g_p1 <- map_dbl(outcome$g, 1)
outcome$g_p2 <- map_dbl(outcome$g, 2)
outcome$g_p3 <- map_dbl(outcome$g, 3)
...
#write.csv(df, file='g_sens.csv')

outcome <- read.csv("g_sens.csv")


p1<-ggplot(data=outcome, aes(x=g_p3, y=p3_fish)) +
  geom_line(size=2) +
  theme_minimal() +
  ...
  labs(x = "Loko i'a fish mortality", y= bquote("Loko i'a fish density"~(g/m^2)))

p2<-ggplot(data=outcome, aes(x=g_p3, y=est_fish)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Loko i'a fish mortality", y= bquote("Bay fish density"~(g/m^2)))

p3<-ggplot(data=outcome, aes(x=g_p3, y=p3_fish_kg)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Loko i'a fish mortality", y= "Total loko i'a fish biomass (kg)")

p4<-ggplot(data=outcome, aes(x=g_p3, y=p1p2_fish_kg)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Loko i'a fish mortality", y= "Total bay fish biomass (kg)")
