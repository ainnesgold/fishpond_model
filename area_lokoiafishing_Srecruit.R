##This script tests loko ia area, loko ia fishing, & recruit fish site fidelity.
library(tidyverse)
library(ggpubr)
library(grid)
library(viridis)
...
#THIS IS WITH ZERO FISHING INSIDE THE LOKO I'A
...
fishing_effort_sequences <- list(0, 0, f_list)
...
parameter_grid <- expand.grid(patch_area = patch_area_list,
                              fishing_effort = fishing_effort_list, S_recruit = S_recruit)
...
for (iter in 1:nrow(parameter_grid)) {
  ...
  recruit_migration <- dispersal(as.numeric(parameter_grid[['patch_area']][[iter]]), number_patches, as.numeric(parameter_grid[['S_recruit']][[iter]]))
...
write.csv(df, file='lokoiafishing_area_sitefidel_juv.csv')

outcome <- read.csv("lokoiafishing_area_sitefidel_juv.csv")
...
#Possible main text figure - set S and S_recruit = 0.5
p1<-ggplot(outcome %>% filter(S_recruit==0.5), aes(x=area_p3*100, y=p3_harv_annual, col=fishing_p3_annual))+
  geom_line(size=2) +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
  #ggtitle("") +
  ...
  labs(x = "Area under loko i'a management (% of bay)", y= "Annual loko i'a harvest (kg)")

p2<-ggplot(outcome %>% filter(S_recruit==0.5), aes(x=area_p3*100, y=est_fish, col=fishing_p3_annual))+
  geom_line(size=2) +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
  #ggtitle("") +
...
p3<-ggplot(outcome %>% filter(S_recruit==0.5), aes(x=area_p3*100, y=p3_fish, col=fishing_p3_annual))+
  geom_line(size=2) +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
  #ggtitle("") +
  theme(plot.title = element_text(size = 20, hjust=0.5), 
        ...
        strip.text.x = element_text(size = 20)) +
  scale_x_continuous(limits = c(0, 10)) +
  labs(x = "Area under loko i'a management", y= bquote("Loko i'a fish density"~(g/m^2)))

#plot<-ggarrange(p1+rremove("xlab"), p3+rremove("xlab"), p2+rremove("xlab"), nrow=1, ncol=3, common.legend = TRUE)

#annotate_figure(plot, bottom=text_grob("Loko i'a area (% of estuary)", size=20))


p4<-ggplot(outcome %>% filter(S_recruit==0.5), aes(x=area_p3*100, y=p1p2_fish_kg, col=fishing_p3_annual))+
  geom_line(size=2) +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
  #ggtitle("") +
...
p5<-ggplot(outcome %>% filter(S_recruit==0.5), aes(x=area_p3*100, y=p3_fish_kg, col=fishing_p3_annual))+
  geom_line(size=2) +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
  #ggtitle("") +
...
plot<-ggarrange(p3+rremove("xlab"), 
                p5+rremove("xlab"), 
                p1 +rremove("xlab"), 
                p2 +rremove("xlab"), 
                p4 +rremove("xlab"), nrow=2, ncol=3, common.legend = TRUE)
...
#supplemental - all S values
ggplot(outcome, aes(x=area_p3*100, y=p3_harv_annual, col=fishing_p3_annual)) +
  geom_line(size=2) +
  facet_wrap(~S_recruit, scales="free") +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
  ggtitle("Site fidelity (recruit)") +
  theme(plot.title = element_text(size = 20, hjust=0.5), 
        axis.title = element_text(size = 20),
        axis.text = element_text(size = 20),
        legend.text = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20)) +
  scale_x_continuous(limits = c(0, 10)) +
  labs(x = "Loko i'a area (% of bay)", y= "Annual loko i'a  harvest (kg)")

#est fish
ggplot(outcome, aes(x=area_p3*100, y=est_fish, col=fishing_p3_annual)) +
  geom_line(size=2) +
  facet_wrap(~S_recruit, scales="free") +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
...
#est fish biomass
ggplot(outcome, aes(x=area_p3*100, y=p1p2_fish_kg, col=fishing_p3_annual)) +
  geom_line(size=2) +
  facet_wrap(~S_recruit, scales="free") +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
...
#loko ia fish density

ggplot(outcome, aes(x=area_p3*100, y=p3_fish, col=fishing_p3_annual)) +
  geom_line(size=2) +
  facet_wrap(~S_recruit, scales="free") +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
...
#loko ia total bioamss
ggplot(outcome, aes(x=area_p3*100, y=p3_fish_kg, col=fishing_p3_annual)) +
  geom_line(size=2) +
  facet_wrap(~S_recruit, scales="free") +
  theme_minimal() +
  scale_color_viridis_d(name="Loko i'a fishing effort")+
  ...
  theme(plot.title = element_text(size = 20, hjust=0.5), 
        ...
        strip.text.x = element_text(size = 20)) +
  scale_x_continuous(limits = c(0, 10)) +
  labs(x = "Loko i'a area (% of bay)", y= "Total bay loko i'a biomass (kg)")
