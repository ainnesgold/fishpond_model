#This script varies Rm (max fish ingestion rate) and Vm (max phyto growth rate).
...
for (iter in 1:nrow(parameter_grid)) {
  ...
  for(t in 2:timesteps){
    ...
    prod[t-1,] = phytoplankton[t-1,] * ((as.numeric(parameter_grid[['Vm']][[iter]]) * nutrients[t-1,])/(ks + nutrients[t-1,]))
    pmort[t-1,] = phytoplankton[t-1,] * m
    grazing[t-1,] = as.numeric(parameter_grid[['Rm']][[iter]]) * ingestion(predation_type, kg, phytoplankton[t-1,]) * fish[t-1,]
...
write.csv(df, file='RmVm_sens.csv')

outcome <- read.csv("RmVm_sens.csv")


#heatmap of sums
ggplot(outcome, aes(x=Vm, y=Rm, fill=est_fish))+
  geom_tile()

ggplot(outcome, aes(x=Vm, y=Rm, fill=est_phyto))+
  geom_tile()

ggplot(outcome, aes(x=Vm, y=Rm, fill=est_nutrients))+
  geom_tile()


#VM EFFECTS - phyto growth rate
#Vm zero was removed bc it led to fish = 0 and we want to zoom in on the effect
p1<-ggplot(data=outcome %>% filter(Rm==1.1 & Vm > 0), aes(x=Vm, y=est_nutrients)) +
  geom_line(size=2) +
  theme_minimal() +
  ...
  labs(x = "Vm", y= bquote('Bay nutrients density'~(g/m^3)))

p2<-ggplot(data=outcome %>% filter(Rm==1.1 & Vm > 0), aes(x=Vm, y=est_phyto)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Vm", y= bquote('Bay phytoplankton density'~(g/m^2)))

p3<-ggplot(data=outcome %>% filter(Rm==1.1 & Vm > 0), aes(x=Vm, y=est_fish)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Vm", y= bquote('Bay fish density'~(g/m^2)))

p4<-ggplot(data=outcome %>% filter(Rm==1.1 & Vm > 0), aes(x=Vm, y=p3_nutrients)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Vm", y= bquote("Loko i'a nutrients density"~(g/m^3)))

p5<-ggplot(data=outcome %>% filter(Rm==1.1 & Vm > 0), aes(x=Vm, y=p3_phyto)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Vm", y= bquote("Loko i'a phytoplankton density"~(g/m^2)))

p6<-ggplot(data=outcome %>% filter(Rm==1.1 & Vm > 0), aes(x=Vm, y=p3_fish)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Vm", y= bquote("Loko i'a fish density"~(g/m^2)))
...
annotate_figure(plot, bottom = text_grob(bquote("Maximum phytoplankton growth rate"~(V[m])), size=20))




#RM EFFECTS - fish ingestion rate
#Rm zero was removed bc it led to fish = 0 and we want to zoom in on the effect
p1<-ggplot(data=outcome %>% filter(Vm==2 & Rm > 0), aes(x=Rm, y=est_nutrients)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Rm", y= bquote('Bay nutrients density'~(g/m^3)))

p2<-ggplot(data=outcome %>% filter(Vm==2 & Rm > 0), aes(x=Rm, y=est_phyto)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Rm", y= bquote('Bay phytoplankton density'~(g/m^2)))

p3<-ggplot(data=outcome %>% filter(Vm==2 & Rm > 0), aes(x=Rm, y=est_fish)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Rm", y= bquote('Bay fish density'~(g/m^2)))

p4<-ggplot(data=outcome %>% filter(Vm==2 & Rm > 0), aes(x=Rm, y=p3_nutrients)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Rm", y= bquote("Loko i'a nutrients density"~(g/m^3)))

p5<-ggplot(data=outcome %>% filter(Vm==2 & Rm > 0), aes(x=Rm, y=p3_phyto)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Rm", y= bquote("Loko i'a phytoplankton density"~(g/m^2)))

p6<-ggplot(data=outcome %>% filter(Vm==2 & Rm > 0), aes(x=Rm, y=p3_fish)) +
  geom_line(size=2) +
  theme_minimal() +
  theme(plot.title = element_text(size = 16), 
        axis.title = element_text(size = 20),
        ...
        strip.text.x = element_text(size = 20), 
        axis.text.x = element_text(vjust = 0.5, hjust=1)) +
  labs(x = "Rm", y= bquote("Loko i'a fish density"~(g/m^2)))
...
#annotate_figure(plot, bottom = text_grob("Maximum fish ingestion rate (Rm)", size=20))
annotate_figure(plot, bottom = text_grob(bquote("Maximum fish ingestion rate"~(R[m])), size=20))
